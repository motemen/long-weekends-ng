...
fetchHolidayEvents({
  ...
  calendarId: "ja.japanese.official#holiday@group.v.calendar.google.com",
})
  .then((holidays) => longWeekends(holidays, { baseDate: today }))
  .then((result) => {
    if (result.active) {
      doPartyHard({
        apiKey: "p40LyUPzKCKD0Sb5AJxzPFUy4kULrHhE41MFnBLHSRYarpPs8M",
      });
      ...
      document
        ...
        .insertAdjacentHTML(
          "afterend",
          '<h1 style="color: black; background: yellow"><marquee><blink>連休発生中です！！！</blink></marquee></h1>'
        );
      tweetButton("連休発生中です！！！");
      return;
    }
    ...
    const template = (
      document.querySelector("#template") as HTMLTextAreaElement
    ).value;
    const dateYMD = function (dt: Date): string {
      const pad0 = (s: any): string => `0${s}`.substr(-2);
      return `${dt.getFullYear()}-${pad0(dt.getMonth() + 1)}-${pad0(
        dt.getDate()
      )}`;
    };
    ...
    for (let i = 1; i < result.items.length; i++) {
      (result.items[i] as any).daysAheadDelta =
        result.items[i].daysAhead -
        result.items[i - 1].daysAhead -
        (result.items[i - 1].holidays.length - 1);
    }
...
const tweetButton: { (text: string): void; done?: boolean } = (
  text: string
) => {
